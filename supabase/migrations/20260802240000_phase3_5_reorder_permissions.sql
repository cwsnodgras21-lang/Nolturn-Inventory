-- Phase 3.5: reorder / restock permissions

insert into public.permissions (key, name, description, category) values
  ('inventory.reorder.read', 'Read reorder rules', 'View reorder rules and restock suggestions', 'inventory'),
  ('inventory.reorder.manage', 'Manage reorder rules', 'Edit reorder rules and create draft POs from restock plans', 'inventory')
...
where r.is_system = true
  ...
  and p.key in ('inventory.reorder.read', 'inventory.reorder.manage')
...
cross join public.permissions p
where r.is_system = true
  and r.key in ('location_manager', 'staff', 'read_only')
  and p.key = 'inventory.reorder.read'
on conflict do nothing;
