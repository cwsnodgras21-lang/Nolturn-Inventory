-- Phase 3.2: purchasing permissions

insert into public.permissions (key, name, description, category) values
  ('purchasing.read', 'Read purchasing', 'View suppliers and purchase orders', 'purchasing'),
  ('purchasing.manage', 'Manage purchasing', 'Create and edit suppliers and purchase orders', 'purchasing'),
  ('purchasing.receive', 'Receive purchase orders', 'Receive inventory against purchase orders', 'purchasing')
...
where r.is_system = true
  ...
  and p.key in ('purchasing.read', 'purchasing.receive')
on conflict do nothing;
