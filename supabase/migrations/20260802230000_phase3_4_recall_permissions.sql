-- Phase 3.4: recall permissions

insert into public.permissions (key, name, description, category) values
  ('inventory.recalls.read', 'Read inventory recalls', 'View recalls, affected lots, and recall history', 'inventory'),
  ('inventory.recalls.manage', 'Manage inventory recalls', 'Create and manage recalls and quarantine affected lots', 'inventory')
...
where r.is_system = true
  ...
  and p.key in ('inventory.recalls.read', 'inventory.recalls.manage')
...
where r.is_system = true
  ...
  and p.key = 'inventory.recalls.read'
on conflict do nothing;
